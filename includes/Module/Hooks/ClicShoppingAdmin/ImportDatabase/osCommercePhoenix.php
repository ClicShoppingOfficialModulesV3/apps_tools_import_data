      ...
      while ($data = $QproductDescriptions->fetch_assoc()) {
        foreach ($clicshopping_languages as $languages) {
          ...
          $sql_data_array = [
            ...
            'products_head_title_tag' => HTML::outputProtected($data['products_seo_title']),
            'products_head_desc_tag' => HTML::outputProtected($data['products_seo_description']),
            'products_head_keywords_tag' => HTML::outputProtected($data['products_seo_keywords'])
          ];
...
// categories_description
        ...
        $sql_data_array = [
          ...
          'categories_head_title_tag' => HTML::outputProtected($data['categories_seo_title']),
          'categories_head_desc_tag' => HTML::outputProtected($data['categories_seo_description'])
        ];
      ...
      while ($data = $QmanufacturersName->fetch_assoc()) {
        foreach ($clicshopping_languages as $languages) {
          $sql_data_array = [
            ...
            'manufacturer_seo_title' => HTML::outputProtected($data['manufacturers_seo_title']),
            'manufacturer_seo_description' => HTML::outputProtected($data['manufacturers_seo_description'])
          ];
...
//******************************************
      $QproductsOption = $mysqli->query('select *
                                       ...
                                       ');
      echo '<hr>';
      echo '<div>products_options_values_to_products_options</div>';
      echo '<div>' . CLICSHOPPING::getDef('text_number_of_item') . ' : ' . $QproductsOption->num_rows . '</div>';
      echo '<hr>';
...
// Prix TTC B2B ----------
            if (($sql_data_array_products_group_price['price' . $QcustomersGroup->valueInt('customers_group_id')] != $Qattributes->valueDecimal('customers_group_price')) && ($Qattributes->valueInt('customers_group_id') == $QcustomersGroup->valueInt('customers_group_id'))) {
